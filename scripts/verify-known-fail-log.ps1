...
.PARAMETER SelfTest
    ...
    have, run this same script against each of them in a child process, and assert its exit code
    AND the failure message it gives -- this gate has three independent ways to refuse, so a plant
    aimed at one of them can go red through another and look like it proved something it did not.
...
'@
    ...
    function New-Lab {
        ...
        Set-LabFile (Join-Path $dir 'Tests/conformance/regenerations.log') (New-LogText $Entries)
    ...
    # SECTION instead of the ENTRY, the case that exists to prove entries past a "## " heading are
    # still compared stayed green -- because the entries had vanished from BOTH sides, so the count
    # check refused instead, and an exit-code-only assertion cannot tell those two refusals apart.
    # The four content gates in this repository already assert their messages for the same reason.
    ...
    Assert-GateRefuses 'known-fail.tsv changed with no new log entry' $lab $CountRefusal
    ...
    Assert-GateRefuses 'the log gutted in a commit that touches no known-fail.tsv' $lab $AppendOnly1
    ...
    # it must accept; cases 12, 13 and 15 are what it must still refuse. This is the one of the
    # three log gates whose real sidecar actually used the exception: all six of
    # Tests/conformance/regenerations.log's pre-existing entries were filled this way.
    $Placeholder = '(no PR yet -- fill in "PR #N" before merging, per CONTRIBUTING.md)'

    # Shape 1: the standalone parenthetical, replaced outright by the number. This is exactly what
    # scripts/regenerate-known-fail.ps1 emits when it is run without -PR.
    $PhEntry2 = @"
2026-01-02 $Placeholder (8 -> 9, 1 more rows) [ephe: 8 files]: Added one row
    that regressed while a second function was being ported, on purpose.
"@
    ...
    Set-LabHead $lab (New-LogText (@($BaseEntries[0], $PhFilled2, $BaseEntries[2]) + $NewEntry4)) -ChangeKnownFail
    ...
    Set-LabHead $lab (New-LogText (@($BaseEntries[0], ($PhFilled2 -replace 'on purpose', 'by accident'), $BaseEntries[2]) + $NewEntry4)) -ChangeKnownFail
    ...
    Set-LabHead $lab (New-LogText (@($BaseEntries[0], ($BaseEntries[1] -replace 'PR #2', 'PR #77'), $BaseEntries[2]) + $NewEntry4)) -ChangeKnownFail
    ...
    $Ph2Entry3 = @'
2026-01-03 (9 -> 7, 2 fewer rows) [ephe: 8 files]: Pruned two newly-passing rows
    after a fidelity fix; the new ones are local (no PR yet, log entry 2; collapsed
...
'@
    ...
    Set-LabHead $lab (New-LogText (@($BaseEntries[0], $BaseEntries[1], $Ph2Filled3) + $NewEntry4)) -ChangeKnownFail
    ...
    Set-LabHead $lab (New-LogText (@($BaseEntries[0], $BaseEntries[1], $Ph2Gutted3) + $NewEntry4)) -ChangeKnownFail
    ...
    $Ph3Entry2 = @'
2026-01-02 PR #13 (8 -> 9, 1 more rows) [ephe: 8 files]: Added one row that
    regressed; the new rows are local (no PR yet, log entry 2).
'@
    ...
    Set-LabHead $lab (New-LogText (@($BaseEntries[0], $Ph3Filled2, $BaseEntries[2]) + $NewEntry4)) -ChangeKnownFail

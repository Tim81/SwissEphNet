...
/// as a whole has not moved off 2.08 yet.</item>
...
/// an unported one -- it is not, by itself, evidence of a defect. Telling that expected mixed
/// state apart from a genuine bug needs a human looking at which components a row exercises, but
/// <see cref="ThreeWayRow.C208VsC210"/> narrows the search: when it reads MATCH, 2.08 C and
/// 2.10.03 C agree with each other and only the port differs, meaning the 2.08-to-2.10.03 C diff
/// never touched this code path at all -- the divergence predates 2.08 and is fixable
/// independently of porting progress.</item>
/// </list>
/// This doc comment deliberately names no counts or specific case_ids: every number it could cite
/// (rows per category, a worked example's exact instance count) is a measurement of one
/// regeneration, and goes stale the moment porting progress moves a row between categories --
/// which is exactly what happened to an earlier version of this comment. Read the current
/// counts from Tests/oracle/version-classification.tsv and -files.tsv themselves (regenerated by
/// scripts/classify-oracle-versions.ps1) and the history of when they changed from
/// Tests/oracle/version-classification-regenerations.log, not from prose here.
/// </summary>
...
internal static class ThreeWayClassificationFile
{
    ...
    /// it is the expected mid-port state, not itself a defect, plus how the c208_vs_c210 column
    /// tells that expected mix apart from a genuine, independently-fixable port bug. Deliberately
    /// carries no worked example with a specific case_id or row count: an earlier version of this
    /// comment cited one (FIXSTAR/FIXSTAR_UT, "32 each"), and it went stale as soon as porting
    /// progress changed which rows fell into that category -- see
    /// Tests/oracle/version-classification-regenerations.log for that history. The file's own data
    /// rows are the current worked example.
    /// </summary>
    ...
    [
        ...
        "# for the classifier. This file is gated: oracle.yml regenerates it and fails on any",
        "# difference from what is committed, so commit what the script produces.",
        "#",
        ...
        "#                   unported one lands here. Telling that expected mix apart from a genuine",
        "#                   bug needs a human reading which components a row exercises -- but the",
        "#                   c208_vs_c210 column narrows the search: MATCH there means 2.08 C and",
        "#                   2.10.03 C agree with each other and only the port differs, so the",
        "#                   2.08-to-2.10.03 C diff never touched this code path and the divergence",
        "#                   predates 2.08, fixable independently of porting progress.",
        "#",
        "# No counts or worked examples are given here on purpose -- read them from this file's own",
        "# rows, which stay current by construction; prose here would not.",
    ];
